...
export default function SITJobSearchApp() {
  ...
  useEffect(() => {
    ...
    socket.on("interview_search_completed", (data: InterviewResults) => {
      console.log("Interview Search Completed:", data)
  ...
  const JobCard = ({ job, index }: { job: Job; index: number }) => {
    ...
    return (
      <motion.div
        className="bg-white/95 backdrop-blur-xl rounded-2xl border-2 border-blue-200 overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300"
      ...
      >
        <div className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 gap-4">
            <div className="flex items-start">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-gray-800 text-lg sm:text-xl mb-1 line-clamp-2">{job.title}</h3>
            ...
            <div className="flex items-center justify-end space-x-3 flex-shrink-0">
              {job.relevance_score && (
                <motion.div
                  className={`px-3 py-1 sm:px-4 sm:py-2 rounded-full bg-gradient-to-r ${getScoreColor(
          ...
          <p className="text-gray-600 mb-4 line-clamp-3 leading-relaxed text-sm sm:text-base">{job.description}</p>

          {job.salary && job.salary !== "Not Disclosed" && (
            <div className="flex items-center text-emerald-600 mb-4 bg-emerald-50 px-3 py-2 rounded-lg">
              <DollarSign className="w-4 h-4 mr-1" />
              <span className="font-semibold text-sm sm:text-base">{job.salary}</span>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <motion.a
              ...
              className="inline-flex items-center px-4 py-2 sm:px-6 sm:py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 text-sm font-bold shadow-lg w-full sm:w-auto justify-center"
        ...
        <div className="p-4 sm:p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1 min-w-0 mr-4">
              ...
              <p className="text-gray-600 mb-2 text-base sm:text-lg font-medium leading-relaxed">{question.question}</p>
              <div className="flex flex-wrap items-center text-gray-600 mb-2 gap-2 sm:gap-4">
                {question.company && (
                  <span className="flex items-center text-sm">
                    <Building className="w-4 h-4 mr-2" />
                    {question.company}
                    {question.year && <span className="ml-2">({question.year})</span>}
                  </span>
                )}
                {question.difficulty && (
                  <span
                    className={`px-2 py-1 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-semibold ${getDifficultyColor(question.difficulty)}`}
                  >
                    {question.difficulty}
                  </span>
                )}
                {question.question_type && (
                  <span className="px-2 py-1 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-semibold bg-blue-100 text-blue-700">
                    {question.question_type}
                  </span>
                )}
              </div>
              {question.source_url && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 text-sm hover:underline flex items-center break-all"
                >
                  <ExternalLink className="w-4 h-4 mr-1 flex-shrink-0" />
                  <span className="truncate">Source: {question.source_title || question.source_url}</span>
                </a>
              )}
              {question.credibility_score && (
                <div className="text-sm text-gray-500 mt-2">Credibility Score: {question.credibility_score}/10</div>
              )}
            </div>
            <button
              onClick={() => toggleQuestionExpansion(index)}
              className="p-2 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors flex-shrink-0"
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                ...
                <div
                  className="leading-relaxed whitespace-pre-wrap markdown-body text-sm sm:text-base"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20">
        <div className="absolute top-0 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-blue-300 rounded-full mix-blend-multiply filter blur-xl animate-pulse"></div>
        <div className="absolute top-0 right-0 w-48 h-48 sm:w-72 sm:h-72 bg-indigo-300 rounded-full mix-blend-multiply filter blur-xl animate-pulse animation-delay-2000"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-cyan-300 rounded-full mix-blend-multiply filter blur-xl animate-pulse animation-delay-4000"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6">
        ...
        >
          <div className="absolute inset-0 overflow-hidden">
            <motion.div
              className="absolute top-1/2 left-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-full blur-3xl"
              animate={{
                scale: [1, 1.2, 1],
                rotate: [0, 180, 360],
              }}
          ...
          <div className="relative z-10">
            ...
            <motion.h1
              className="text-4xl sm:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-blue-700 via-indigo-600 to-blue-800 bg-clip-text text-transparent mb-4 sm:mb-6 tracking-tight"
            ...
            >
              SymbiSpark
              <motion.span
                className="block text-2xl sm:text-4xl lg:text-5xl mt-2 bg-gradient-to-r from-indigo-600 to-blue-800 bg-clip-text text-transparent"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.6, duration: 0.8 }}
              >
                INTELLIGENT CAREER SEARCH PLATFORM
              </motion.span>
            </motion.h1>
        ...
        >
          <div
            className={`flex items-center px-4 py-2 sm:px-6 sm:py-3 rounded-full backdrop-blur-md border-2 shadow-lg ${
          ...
          >
            ...
            <span className="font-bold text-sm sm:text-base">{connectionStatus}</span>
          </div>
        </motion.div>

        <motion.div
          className="bg-white/90 backdrop-blur-xl rounded-3xl border-2 border-blue-200 p-4 sm:p-8 mb-6 sm:mb-8 shadow-2xl"
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex mb-6">
            <motion.button
              className={`flex-1 px-4 py-3 sm:px-6 rounded-l-xl font-bold text-base sm:text-lg ${
                activeTab === "jobs"
            ...
            <motion.button
              className={`flex-1 px-4 py-3 sm:px-6 rounded-r-xl font-bold text-base sm:text-lg ${
                activeTab === "interview"
                  ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white"
                  : "bg-gray-200 text-gray-700"
              }`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <motion.div whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 300 }}>
              {activeTab === "jobs" ? (
                <>
                  <div className="bg-gradient-to-br from-blue-600 to-indigo-600 p-4 sm:p-6 rounded-2xl text-white mb-6 shadow-xl">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 sm:py-4 border-2 border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-base sm:text-lg font-semibold text-black"
                    ...
                    {showFilters && (
                      <motion.div
                        className="grid grid-cols-1 gap-4 p-4 sm:p-6 bg-blue-50 rounded-2xl border-2 border-blue-200"
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        transition={{ duration: 0.3 }}
                      >
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-3 py-3 border-2 border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-500 font-semibold"
                          >
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <motion.button
                        onClick={searchJobs}
                        disabled={loading}
                        className="flex-1 px-6 py-3 sm:px-8 sm:py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-bold hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-all duration-300 border-2 border-blue-400 shadow-xl flex items-center justify-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                        {loading ? (
                      ...
                      <motion.button
                        ...
                        className="px-4 py-3 sm:px-6 sm:py-4 bg-slate-600 text-white rounded-2xl font-bold hover:bg-slate-700 transition-all duration-300 border-2 border-slate-500 shadow-xl flex items-center justify-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {popularRoles.slice(0, 6).map((role) => (
                          <button
                            ...
                            className="px-3 py-2 bg-blue-100 text-blue-700 rounded-full text-xs sm:text-sm hover:bg-blue-200 transition-colors font-semibold border border-blue-300"
                          >
                            {role}
                          </button>
                        ))}
                ...
                <>
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <motion.button
                      ...
                      className="w-full px-6 py-3 sm:px-8 sm:py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-bold hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-all duration-300 border-2 border-blue-400 shadow-xl flex items-center justify-center"
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      {interviewLoading ? (
            ...
            >
              <div className="relative w-[350px] h-[650px] bg-slate-900 rounded-[40px] p-3 shadow-2xl border-4 border-slate-700">
          ...
          <AnimatePresence>
            <motion.div
              className="mt-6 p-4 sm:p-6 rounded-2xl bg-slate-50 border-2 border-slate-200"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
            >
              <div className="flex items-center">
                {error ? (
                  ...
                  >
                    <Activity className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 mr-3" />
                  </motion.div>
                )}
                <p
          ...
          {error && (
            <motion.div
              className="mt-4 p-4 bg-red-50 text-red-700 rounded-lg flex items-center border-2 border-red-200"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.5 }}
            >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-1 gap-6 sm:gap-8">
          {activeTab === "jobs" ? (
            <AnimatePresence>
              {searchResults && (
                <motion.div
                  className="bg-white/95 backdrop-blur-xl rounded-3xl border-2 border-blue-200 overflow-hidden shadow-2xl"
                  initial={{ opacity: 0, y: 50 }}
                  animate={{ opacity: 1, y: 0 }}
                    ...
                    {showCandidatesAnimation ? (
                      ...
                      >
                        <div className="text-center">
                          <motion.div
                            className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-blue-500 border-t-transparent rounded-full mx-auto mb-6"
                            animate={{ rotate: 360 }}
                          ...
                          <p className="text-blue-600 font-semibold">Presenting career opportunities...</p>
                        </div>
                      </motion.div>
                      ...
                      >
                        <motion.div
                          className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-blue-200 to-indigo-200 rounded-full flex items-center justify-center mx-auto mb-8"
                          animate={{ scale: [1, 1.1, 1] }}
                          transition={{ duration: 2, repeat: Number.POSITIVE_INFINITY }}
                        >
                          <Search className="w-10 h-10 sm:w-12 sm:h-12 text-blue-600" />
                        </motion.div>
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">NO MATCHES FOUND</h3>
              ...
              {interviewLoading ? (
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-blue-600 font-semibold">Analyzing interview questions...</p>
                  </div>
                </motion.div>
              ) : interviewResults ? (
                <motion.div
                ...
                >
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 sm:p-6 border-b-2 border-blue-300">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      ...
                      <motion.button
                        onClick={downloadPDF}
                        className="px-4 py-2 sm:px-6 sm:py-3 bg-white text-blue-600 rounded-xl font-bold hover:bg-blue-100 transition-all duration-300 border-2 border-blue-400 shadow-lg flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                    ...
                    {interviewResults.search_metadata ? (
                      ...
                      <p className="text-blue-100 text-sm">Metadata unavailable</p>
                    )}
                  </div>
                  ...
                  >
                    {interviewResults.questions.length === 0 ? (
                      ...
                      >
                        ...
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4">NO QUESTIONS FOUND</h3>
                        <p className="text-gray-600 text-base sm:text-lg">
        ...
        {activeTab === "jobs" && savedJobs.length > 0 && (
          <motion.div
            className="mt-6 sm:mt-8 bg-white/95 backdrop-blur-xl rounded-3xl border-2 border-blue-200 overflow-hidden shadow-2xl"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
