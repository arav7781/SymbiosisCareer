...
export default function SITJobSearchApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (response.ok) {
          ...
          const list: string[] = isStringArray(data)
            ? data
            : hasSuggestions(data)
            ? data.suggestions
            : [];
    ...
    socket.on("question_extracted", (data: {
      question: string;
      source: string;
      company?: string;
      difficulty?: string;
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center">
              <motion.div
                className="w-12 h-12 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold mr-4 text-lg shadow-lg"
              ...
              <div>
                <h3 className="font-bold text-gray-800 text-xl mb-1">{job.title}</h3>
            ...
            <div className="flex items-center space-x-3">
              {job.relevance_score && (
                <motion.div
                  className={`px-4 py-2 rounded-full bg-gradient-to-r ${getScoreColor(
          ...
          <div className="flex items-center text-sm text-gray-500 mb-4">
            ...
            <span className="mr-4">{job.experience_level || "Entry Level"}</span>
            <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-3 py-1 rounded-full text-xs font-bold">
              {job.source}
            </span>
          </div>

          <p className="text-gray-600 mb-4 line-clamp-3 leading-relaxed">{job.description}</p>
          ...
          <div className="flex justify-between items-center">
            ...
            <motion.a
              ...
              className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 text-sm font-bold shadow-lg"
    ...
    return (
      ...
      >
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="font-bold text-gray-800 text-xl mb-1">Question {index + 1}</h3>
              <p className="text-gray-600 mb-2 text-lg font-medium">{question.question}</p>
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                ...
                <div
                  className="leading-relaxed whitespace-pre-wrap markdown-body"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6">
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-center mb-8">
              ...
              >
                <div className="w-20 h-20 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full flex items-center justify-center shadow-2xl border-4 border-white">
                  <GraduationCap className="w-10 h-10 text-white" />
                </div>
                <div className="absolute inset-0 w-20 h-20 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full animate-ping opacity-20"></div>

                <motion.div
                  className="absolute top-1/2 left-1/2 w-32 h-32 border-2 border-blue-300/30 rounded-full"
                  style={{ transform: "translate(-50%, -50%)" }}
                  animate={{ rotate: -360 }}
                ...
                <motion.div
                  className="absolute top-1/2 left-1/2 w-40 h-40 border border-indigo-300/20 rounded-full"
                  style={{ transform: "translate(-50%, -50%)" }}
                  animate={{ rotate: 360 }}
            ...
            <motion.h1
              className="text-7xl font-bold bg-gradient-to-r from-blue-700 via-indigo-600 to-blue-800 bg-clip-text text-transparent mb-6 tracking-tight"
            ...
            >
              SymbiSpark
              <motion.span
                className="block text-5xl mt-2 bg-gradient-to-r from-indigo-600 to-blue-800 bg-clip-text text-transparent"
        ...
        >
          <div
            className={`flex items-center px-6 py-3 rounded-full backdrop-blur-md border-2 shadow-lg ${
        ...
        <motion.div
          className="bg-white/90 backdrop-blur-xl rounded-3xl border-2 border-blue-200 p-8 mb-8 shadow-2xl"
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <motion.div whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 300 }}>
              {activeTab === "jobs" ? (
                <>
                  <div className="bg-gradient-to-br from-blue-600 to-indigo-600 p-6 rounded-2xl text-white mb-6 shadow-xl">
                    <div className="flex items-center mb-4">
                      <Search className="w-8 h-8 mr-4" />
                      <h2 className="text-2xl font-bold">INTELLIGENT JOB SEARCH</h2>
                    </div>
                    <p className="text-blue-100 text-lg">Advanced AI-powered career matching for SIT professionals</p>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-4 border-2 border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-lg font-semibold text-black"
                    ...
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-wide">Location</label>
                      <select
                        value={location}
                    ...
                    {showFilters && (
                      <motion.div
                        className="grid grid-cols-1 gap-4 p-6 bg-blue-50 rounded-2xl border-2 border-blue-200"
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        transition={{ duration: 0.3 }}
                    ...
                    <div className="flex flex-wrap gap-4">
                      <motion.button
                        onClick={searchJobs}
                        disabled={loading}
                        className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-bold hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-all duration-300 border-2 border-blue-400 shadow-xl flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                        {loading ? <Loader2 className="w-6 h-6 mr-3 animate-spin" /> : <Search className="w-6 h-6 mr-3" />}
                        {loading ? "PROCESSING..." : "START SEARCH"}
                      </motion.button>

                      <motion.button
                        onClick={() => setShowFilters(!showFilters)}
                        className="px-6 py-4 bg-slate-600 text-white rounded-2xl font-bold hover:bg-slate-700 transition-all duration-300 border-2 border-slate-500 shadow-xl flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {popularRoles.slice(0, 6).map((role) => (
                          <button
                            ...
                            className="px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm hover:bg-blue-200 transition-colors font-semibold border border-blue-300"
                          >
                            {role}
                          </button>
                        ))}
                ...
                <>
                  <div className="bg-gradient-to-br from-blue-600 to-indigo-600 p-6 rounded-2xl text-white mb-6 shadow-xl">
                    <div className="flex items-center mb-4">
                      ...
                      <h2 className="text-2xl font-bold">INTERVIEW QUESTION SEARCH</h2>
                    </div>
                    <p className="text-blue-100 text-lg">Find India-specific interview questions for your domain</p>
                  </div>
          ...
          <AnimatePresence>
            <motion.div
              className="mt-6 p-6 rounded-2xl bg-slate-50 border-2 border-slate-200"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
            >
          ...
          {activeTab === "jobs" ? (
            <AnimatePresence>
              {searchResults && (
                ...
                >
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 border-b-2 border-blue-300">
                    <h2 className="text-3xl font-bold text-white flex items-center">
                      <Shield className="w-8 h-8 mr-4 text-white" />
                  ...
                  <AnimatePresence mode="wait">
                    {showCandidatesAnimation ? (
                      ...
                      >
                        <motion.div
                          className="w-24 h-24 bg-gradient-to-br from-blue-200 to-indigo-200 rounded-full flex items-center justify-center mx-auto mb-8"
                      ...
                      <motion.div
                        className="p-6 space-y-6 max-h-[800px] overflow-y-auto"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.5 }}
                        key="jobs"
              ...
              {interviewLoading ? (
                ...
                >
                  <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 border-b-2 border-blue-300">
                    <div className="flex justify-between items-center">
                      <h2 className="text-3xl font-bold text-white flex items-center">
                        <FileText className="w-8 h-8 mr-4 text-white" />
                        INTERVIEW QUESTIONS
                      </h2>
                      <motion.button
                        onClick={downloadPDF}
                        className="px-6 py-3 bg-white text-blue-600 rounded-xl font-bold hover:bg-blue-100 transition-all duration-300 border-2 border-blue-400 shadow-lg flex items-center"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
                    ...
                    {interviewResults.search_metadata ? (
                      <p className="text-blue-100 text-sm">
                        Sources searched: {interviewResults.search_metadata.sources_searched || 0} | Queries used: {interviewResults.search_metadata.search_queries_used || 0}
                      </p>
                    ) : (
        ...
        {activeTab === "jobs" && savedJobs.length > 0 && (
          <motion.div
            className="mt-8 bg-white/95 backdrop-blur-xl rounded-3xl border-2 border-blue-200 overflow-hidden shadow-2xl"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 p-6 border-b-2 border-indigo-300">
              <h2 className="text-3xl font-bold text-white flex items-center">
                <Bookmark className="w-8 h-8 mr-4 text-white" />
